...
const MainLayout = ({ children }) => {
  ...
  const menuItems = [
    ...
    { name: 'Notifications', icon: Bell, path: getRoleModulePath('notifications'), roles: [roles.ADMIN, roles.MANAGER, roles.CHEF, roles.WAITER, roles.CASHIER] },
                ...
                <>
                  ...
                  <div className="fixed top-16 right-4 sm:right-8 w-40 bg-surface border border-slate-100 rounded-2xl shadow-2xl overflow-hidden z-[510] animate-in fade-in slide-in-from-top-2 duration-200 p-1">
                    <button
                      ...
                      className="w-full flex items-center gap-3 p-3 hover:bg-rose-50 rounded-xl transition-all text-slate-400 hover:text-primary group"
                    >
